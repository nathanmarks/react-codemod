...
module.exports = (file, api, options) => {
  ...
  const createBoundMethodDefinition = fn =>
  ...
  const createStateClassField = getInitialState => {
    if (!getInitialState) {
      return [];
    }
  ...
  const createClassProperties = (statics, getInitialState) =>
    statics.map(createStaticClassProperty)
      .concat(createStateClassField(getInitialState));
  ...
  const updateToClass = (classPath, type) => {
    ...
    const properties = createClassProperties(statics, getInitialState);

    path.replaceWith(
      createESClass(
        name,
        properties.concat(functions.map(n => {
          if (autobindFunctions.indexOf(n.key.name) !== -1) {
            return createBoundMethodDefinition(n);
          }
